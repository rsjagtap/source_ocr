...
class ExtractSingleText{
	...
	ExtractSingleText();
	~ExtractSingleText();
	void createFolder4CroppedText(string& dataSetPath);
	void initilizeTesseract(tesseract::TessBaseAPI* api);
	bool binarizeLP(string& imageToSave, Mat& binaryImage , Mat& grayImage, Mat& threholdImage, string& dataSetPath,char*  imageToSave_convert);
	...
	void drawRectOverSingleText(Mat& binaryImage, std::vector<std::vector<cv::Point> > &contours,int& const_y1,int& const_y2,int& letter_count,int& count_crop, Mat& drawing,/* vector<vector<Point> > &contours_poly, vector<Rect> &boundRect, vector<Rect> &boundRectCrop, vector<Point2f> &center, vector<float> &radius,*/ Mat& threholdImage, int& count_temp, char imageToSave_crop[]);
